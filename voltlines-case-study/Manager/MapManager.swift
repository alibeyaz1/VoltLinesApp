...
class MapManager {
        ...
        APIManager.sharedManager.getStationList { (status) in
            if status.count > 0 {
                let stationList = status.map { stationItem in
                    if let selectedStation = stationItem.trips.first(where: {$0.idBus == self.bookedTrip}) {
                    ...
                    return MapStation(
                        ...
                        booked: (stationItem.trips.first(where: {$0.idBus == self.bookedTrip}) != nil)
                    )
    ...
    func fetchSelectedStationCoords(_ id: Int) -> CLLocation {
        if bookedStation != nil, let bookedId = bookedStation?.idBus {
